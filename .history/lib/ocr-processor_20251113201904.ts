...
export class OCRProcessor {
  ...
  private parseInvoiceFields(text: string): OCRResult["extractedFields"] {
    ...
    // 銀行名の抽出（改善版）
    // パターン1: 「○○銀行」という形式を探す
    const bankNamePattern = /([ぁ-んァ-ヶー一-龠]{2,10}銀行)/
    const bankMatch = text.match(bankNamePattern)
    
    if (bankMatch) {
      const bankName = bankMatch[1].replace(/\s+/g, "")
    ...
    // 支店名の抽出（改善版）
    // パターン1: 「○○支店」という形式を探す（銀行名の後）
    let branchNamePattern = /([ぁ-んァ-ヶー一-龠]{2,10}(?:支店|支所))/
    
    // 銀行名が見つかった場合、その後ろから支店名を探す
    if (fields.bankName) {
      const bankNameIndex = text.indexOf(fields.bankName.value)
      if (bankNameIndex !== -1) {
        const textAfterBank = text.substring(bankNameIndex + fields.bankName.value.length)
        const branchMatch = textAfterBank.match(branchNamePattern)
        if (branchMatch) {
          const branchName = branchMatch[1].replace(/\s+/g, "")
